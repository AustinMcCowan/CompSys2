...
int do_client(char*, char*, string);
...
int main(int argc, char *argv[])
{
  ...
  command += '\n';
  // invoke client code
  return do_client(argv[1], argv[2], command);
...
int do_client(char *server, char *port, string cmd)
{
  ...
  // Attempt to transfer data to server
  char *cbuff = (char *)cmd.c_str();

  cout << "ATTEMPT TO SEND DATA" << endl;

  int needed=cmd.length();
  cout << needed << endl;
  while(needed) {
